...
related_specs: [SPEC-UI-001, SPEC-FE-001, SPEC-FE-002, SPEC-FE-004]
---
...
| Q&A Popup | design/screens/qa/qa-popup.pen | Inline popup triggered by text selection in material viewer |
...
apps/web/                          # Next.js 15 (this SPEC)
  app/(dashboard)/qa/              # Q&A list and detail pages
  components/qa/                   # Q&A-specific components
  stores/qa.store.ts               # Q&A client state (Zustand)
...
packages/shared/
  ...
  src/validators/qa.schema.ts      # Q&A Zod schemas
  src/constants/events.ts          # WebSocket event name constants
```
...
- **Markdown**: Questions and answers render and accept input in Markdown format
...
- **Text Selection Context**: Captures material ID, section anchor, and selected text content
...
- SPEC-FE-001 (foundation) is complete and provides:
  ...
  - TanStack Query `QueryProvider` and `api.ts` API client
  - Zustand store infrastructure and devtools setup
  - `packages/shared/src/types/qa.types.ts` skeleton file

### 2.2 Auth Dependency (SPEC-FE-002)

- SPEC-FE-002 (auth) is complete and provides:
  - `useSession()` hook exposing `{ user: { id, role, name, avatarUrl } }`
  - `UserRole` enum (`INSTRUCTOR`, `STUDENT`) from `@shared/types/auth.types`
  - Authenticated API client with Bearer token injection

### 2.3 Material Viewer Dependency (SPEC-FE-004)

- SPEC-FE-004 (material viewer) provides:
  - `useTextSelection()` hook that returns `{ selectedText, anchorRect, materialId, sectionAnchor }` on user text selection
  - `MaterialViewer` component exposes a `onSelectionIntent` callback triggered when user releases mouse/touch after selecting text
  - Text selection state is managed locally in the material viewer; SPEC-FE-006 subscribes via callback or event
  - Material page URL structure: `/courses/[courseId]/materials/[materialId]`

### 2.4 Backend API Assumptions
...
- AI answer endpoint proxied through backend: `POST /api/v1/qa/questions/[id]/ai-suggest`
- WebSocket events delivered via existing `@fastify/websocket` connection

### 2.5 AI Service Assumptions
...
- Real-time WebSocket server implementation is out of scope; frontend prepares the client-side connection skeleton and polling fallback
- Full-text search across Q&A is out of scope for this iteration (search UI included, backed by backend `q` param)
...
The system shall enrich `packages/shared/src/types/qa.types.ts` with complete Q&A domain types used across frontend and backend.

Required types:

```
QAStatus            - enum: OPEN | RESOLVED | CLOSED
QAQuestionContext   - { materialId, sectionAnchor, selectedText }
QAQuestion          - { id, courseId, materialId, authorId, author, title, content (markdown),
                        context: QAQuestionContext, status, upvoteCount, isUpvoted,
                        answerCount, aiSuggestion?, createdAt, updatedAt }
QAAnswer            - { id, questionId, authorId, author, content (markdown), isAccepted,
                        isAiGenerated, upvoteCount, isUpvoted, createdAt, updatedAt }
QAListItem          - subset of QAQuestion for list rendering (no full content)
QACreateRequest     - { courseId, materialId, title, content, context: QAQuestionContext }
QAAnswerRequest     - { content: string }
QAListFilter        - { courseId?, materialId?, status?, q?, page, limit }
QAAuthorInfo        - { id, name, avatarUrl, role: UserRole }
```
...
The system shall add Q&A-specific WebSocket event constants to `packages/shared/src/constants/events.ts`.

Required constants:

- `QA_EVENTS.NEW_QUESTION` — broadcast when a new question is posted in a course
- `QA_EVENTS.NEW_ANSWER` — broadcast when an answer is posted on a question
- `QA_EVENTS.AI_SUGGESTION_READY` — broadcast when AI suggestion is available
- `QA_EVENTS.QUESTION_RESOLVED` — broadcast when question status changes to RESOLVED
...
- `openInlinePopup(anchorRect, context)` — triggered by text selection
...
#### REQ-FE-510: Text Selection Trigger

**When** a user selects text in the material viewer and releases the mouse or touch, **then** the system shall display the inline Q&A popup anchored to the selection position.

- Integration point: `MaterialViewer` `onSelectionIntent` callback (from SPEC-FE-004)
- Trigger condition: selected text length >= 5 characters
- Popup opens via `qa.store.openInlinePopup(anchorRect, context)`
- Context captured: `{ materialId, sectionAnchor, selectedText }`
...
- Sections:
  ...
  3. Question content editor — Markdown textarea with preview toggle
...
- `role="dialog"` with `aria-modal="true"` and `aria-label="질문하기"` (or locale-based label)
...
- Fields:
  ...
  - `content` (required, min 20 chars) — Markdown editor with preview toggle
...
#### REQ-FE-514: Markdown Editor in Popup

**Where** the content editor is rendered in the inline popup, the system shall provide a simple Markdown editor with preview toggle.

- `apps/web/components/qa/MarkdownEditor.tsx` — reusable editor component
- Two-tab UI: "Write" tab (textarea) and "Preview" tab (react-markdown renderer)
- Toolbar with minimal formatting shortcuts: Bold, Italic, Code, Link, Unordered List
- Character counter for content field
- Preview uses same react-markdown configuration as material viewer (remark-gfm, syntax highlighting)
...
- Validation: `context.selectedText` must be non-empty
...
- Updates via WebSocket `QA_EVENTS.NEW_ANSWER` and `QA_EVENTS.AI_SUGGESTION_READY` events
...
- Displayed content:
  ...
  - Context block: styled blockquote with material name + section anchor link + selected text
  - Question body: rendered Markdown (react-markdown, full feature set)
...
- `MarkdownEditor` component (from REQ-FE-514)
- Submit button: "답변 등록"
...
- Subscribe to `QA_EVENTS.NEW_ANSWER` WebSocket event for the active question
...
- Subscribe to `QA_EVENTS.AI_SUGGESTION_READY` WebSocket event
...
The system shall establish a WebSocket connection to receive Q&A events when the user is authenticated.

- `apps/web/lib/ws-client.ts` — singleton WebSocket client (skeleton may exist from FE-001; this SPEC extends it for Q&A events)
- `apps/web/hooks/qa/useQAWebSocket.ts` — React hook subscribing to Q&A events
- Connection lifecycle managed in root layout (or `QueryProvider`)
- Reconnection logic: exponential backoff, max 5 retries

#### REQ-FE-546: In-App Toast Notifications

**When** the user receives a `QA_EVENTS.NEW_ANSWER` event for a question they authored, **then** the system shall show a Toast notification.
...
**When** the user receives a `QA_EVENTS.AI_SUGGESTION_READY` event, **then** the system shall show a Toast notification.
...
- On `NEW_ANSWER` event: add to store if user is not currently viewing that question
- On `AI_SUGGESTION_READY` event: add to store if user is not currently viewing that question
...
The system shall not display raw Markdown syntax to end users; all Markdown content must be rendered through react-markdown before display.

#### REQ-FE-N506: No Popup on Closed Questions
...
apps/web/
  app/
    (dashboard)/
      qa/
        page.tsx                          # Q&A list page (Server Component)
        layout.tsx                        # Q&A section layout (optional)
        loading.tsx                       # Q&A list skeleton
        [questionId]/
          page.tsx                        # Q&A detail page (Server Component)
          loading.tsx                     # Q&A detail skeleton
          not-found.tsx                   # 404 for invalid question ID

  components/qa/
    QAInlinePopup.tsx                     # Inline popup triggered by text selection
    ...
    MarkdownEditor.tsx                    # Reusable Markdown write/preview editor
  ...
  lib/
    ws-client.ts                          # WebSocket singleton client (extended)

  stores/
    qa.store.ts                           # Zustand: popup, notifications, WS state
...
packages/shared/
  src/
    ...
    validators/
      qa.schema.ts                        # Q&A Zod schemas
    constants/
      events.ts                           # QA_EVENTS constants added
```

### 4.2 Q&A Type Definitions (packages/shared/src/types/qa.types.ts)
...
export interface QAQuestionContext {
  materialId: string;
  sectionAnchor: string;     // heading anchor or paragraph ID
...
```
1. Get anchorRect from text selection
2. Calculate popup preferred position: { top: anchorRect.bottom + 8, left: anchorRect.left }
...
#### MarkdownEditor Component Interface
...
### 4.6 WebSocket Event Payload Shapes

```typescript
// QA_EVENTS.NEW_ANSWER
{
  type: 'NEW_ANSWER',
  questionId: string,
  answerId: string,
  actorName: string,
  questionTitle: string,
  recipientIds: string[],    // server sends to correct users only
}

// QA_EVENTS.AI_SUGGESTION_READY
{
...
|------------|-----------------|----------------------|
| REQ-FE-510 (Text Selection Trigger) | design/screens/qa/qa-popup.pen | SPEC-FE-004 `onSelectionIntent` |
...
| REQ-FE-514 (Markdown Editor) | design/screens/qa/qa-popup.pen | SPEC-FE-004 react-markdown setup |
...
| REQ-FE-534 (Answer Form) | design/screens/qa/qa-detail.pen | SPEC-FE-001 Form |
...
| REQ-FE-545 (WebSocket) | — | SPEC-FE-001 ws-client skeleton |
| REQ-FE-546 (Toast Notifications) | design/screens/common/notification.pen | SPEC-FE-001 Toast (Sonner) |
| REQ-FE-500 (Type Definitions) | — | SPEC-FE-001 `@shared/types/qa.types` skeleton |
| REQ-FE-502 (WS Events) | — | SPEC-FE-001 `@shared/constants/events` |
| REQ-FE-540 (Moderation) | design/screens/qa/qa-detail.pen | SPEC-FE-002 role check |
